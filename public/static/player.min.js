...
// ⚡ Entrar "fullscreen" (simulado) + horizontal
async function enterFullscreen() {
  try {
    // 2️⃣ Simular fullscreen ocultando header y scroll
...
// ⚡ Salir "fullscreen" + restaurar orientación
...
function setOverlayTitle() {
  ...
  // espacio disponible para el título (30px extra de separación)
  ...
  // búsqueda binaria para recortar inteligentemente
  let left = 0;
  ...
  while (left <= right) {
    ...
    if (temp.offsetWidth <= available) {
      result = testStr;
      left = mid + 1;   // intentar con más caracteres
    } else {
...
async function loadServerByIndex(index) {
  ...
  try {
    ...
    if (['yu', 'yourupload', 'bc', 'burcloud', 'sw', 'voe', 'stape'].includes(server.servidor)) {
      const serverNames = {
        'yu': 'yu',
        'yourupload': 'yu',
        'bc': 'bc',
      ...
      try {
        ...
        if (serverParam === 'sw' || serverParam === 'voe') {
          const json = await res.json();
          const response = await fetch(json.mediaurl);
...
async function searchSuggestionsInput() {
  ...
  try {
    ...
    // Filtrar resultados de Kitsu por términos
    const apiFiltered = data.data
      ...
      .map(a => {
        // Buscar coincidencia en la lista local por cualquiera de sus títulos
    ...
    // Combinar locales + API, sin duplicados, máximo 4
...
async function searchAnime(event = null, term = null) {
  ...
  console.log("Búsqueda ingresada:", input);
  console.log("Búsqueda normalizada:", inputNormalized);
  ...
  // Si no hay término de búsqueda, mostramos toda la lista local
  if (!inputNormalized) {
    const page = getPageParam();
    const paginated = paginate(fullAnimeList, page);
    console.log("No hay término de búsqueda, mostrando lista completa, página:", page);
  ...
  console.log("Términos de búsqueda:", terms);

  // 1. Petición a la API de Kitsu
  let apiResults = [];
  try {
    ...
    // 2. Filtrar resultados de la API por coincidencia en cualquier título
    apiResults = data.data
      .filter(a => {
        ...
        // Retorna true si **todos los términos de búsqueda** aparecen en algún título
        return terms.every(term => titleVariants.some(t => t.includes(term)));
      })
