async function initPlayerPage() {
  ...
  try {
    ...
    if (ep > 1) {
      ...
      if (id) {btnPrev.onclick = () => location.href = `/player?id=${encodeURIComponent(id)}&ep=${ep - 1}`}else{btnPrev.onclick = () => location.href = `/player?uid=${encodeURIComponent(uid)}&ep=${ep - 1}`}
    ...
    if (ep < data.episodes_count) {
      ...
      if (id) {btnNext.onclick = () => location.href = `/player?id=${encodeURIComponent(id)}&ep=${ep + 1}`}else{btnNext.onclick = () => location.href = `/player?uid=${encodeURIComponent(uid)}&ep=${ep + 1}`}
    ...
    function renderEpisodeGroup() {
      ...
      for (let i = start; i <= end; i++) {
        ...
        if (id) {btn.href = `/player?id=${encodeURIComponent(id)}&ep=${i}`}else{btn.href = `/player?uid=${encodeURIComponent(uid)}&ep=${i}`}
