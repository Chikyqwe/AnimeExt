 ...
 */
let container, mainContent, recolectForm, pageTitle, suggestionBox, hamburgerBtn, mobileDropdownMenu, login;
 ...
 * =======================================================
 */
function versionStringToCode(versionStr) {
...
function onBackKeyDown(e) {
    ...
    // ----------------------------------------
    // 1. Cerrar cualquier modal de Bootstrap
    // ----------------------------------------
    ...
    // ----------------------------------------
    // 2. Si NO hay modal → mostrar menú de salida
    // ----------------------------------------
    const exitMenu = document.getElementById('modalExit');
    if (exitMenu) {
        exitMenu.classList.remove('d-none');
        setupExitMenu();
        return;
    }
}
...
// ----------------------------------------
// Configurar botones del menú de salida
// ----------------------------------------
function setupExitMenu() {
    ...
    // Botón cancelar → cerrar overlay
    cancelBtn.onclick = function () {
        cerrarOverlay('modalExit');
    };

    // Botón salir → cerrar app REAL en Cordova
...
// ----------------------------------------
// Cerrar overlay del menú de salida
// ----------------------------------------
...
function mainInit() {
    ...
    if (login) login.addEventListener('click', () => alert("Fuction in development"));
    ...
    if (mobileNavInicio) mobileNavInicio.addEventListener('click', () => layout(1), toggleMobileMenu(false));
    const mobileNavDirectorio = document.getElementById('mobileNavDirectorio');
    if (mobileNavDirectorio) mobileNavDirectorio.addEventListener('click', () => layout(2), toggleMobileMenu(false));
    const mobileNavHistorial = document.getElementById('mobileNavHistorial');
    if (mobileNavHistorial) mobileNavHistorial.addEventListener('click', () => layout(3), toggleMobileMenu(false));
    const mobileNavFavoritos = document.getElementById('mobileNavFavoritos');
    if (mobileNavFavoritos) mobileNavFavoritos.addEventListener('click', () => layout(4), toggleMobileMenu(false));
    const mobileNavSubs = document.getElementById('mobileNavSubs');
    if (mobileNavSubs) mobileNavSubs.addEventListener('click', () => layout(5), toggleMobileMenu(false));
        ...
        .then(data => {
            ...
            data.forEach(anime => {
                ...
                const createBar = () => {
                    ...
                    bar.className = "longpress-bar";
                    bar.style.transition = "none";
                    bar.style.width = "0%";
                    card.appendChild(bar);

                    // Forzar reflow para permitir animación
                ...
                const startHold = () => {
                    ...
                    createBar();
                    holdTimer = setTimeout(openInfo, 1500);
                };
                ...
                // Click normal solo si NO hubo pulsación larga
                card.onclick = () => {
...
async function layout(num, e) {
    ...
    document.getElementById("subs-section").classList.add("d-none");
    
    // Sidebar y paginación por defecto
    ...
    switch (num) {
        ...
        // ============================================================
        case 3:
            const containerH = document.getElementById('historial-container');
            document.getElementById("historial-section").classList.remove("d-none");

            if (containerH) {
                containerH.innerHTML = '<div class="text-white-50">Cargando historial...</div>';
            }
            ...
            try {
                ...
                if (!history.length) {
                    containerH.innerHTML = '<p class="text-white-50">No hay historial disponible.</p>';
                    hideLoader();
                    return;
                }
                    ...
                    card.innerHTML = `
                        ...
                        <div class="anime-overlay">
                            <button class="btn-remove">
                                <i class="bi bi-trash"></i> Quitar del historial
                            </button>
                ...
                containerH.innerHTML = '<p class="text-danger">Error al cargar historial.</p>';
            }
        ...
        // ============================================================
        case 4:
            const containerF = document.getElementById('favoritos-container');
            document.getElementById("favoritos-section").classList.remove("d-none");

            if (containerF) {
                containerF.innerHTML = '<div class="text-white-50">Cargando favoritos...</div>';
            }
            ...
            try {
                ...
                if (!favoritosData.length) {
                    containerF.innerHTML = '<p class="text-white-50">Aún no has agregado animes a favoritos.</p>';
                    hideLoader();
                    return;
                }
                ...
                favoritosData.forEach(anime => {
                    containerF.appendChild(createFavoriteCard(anime));
                });

            } catch (err) {
                containerF.innerHTML = '<p class="text-danger">Error mostrando favoritos.</p>';
            }

            setTimeout(() => hideLoader(), 1000);
            return;
        case 5:
            const containerS = document.getElementById('subs-container');
            document.getElementById("subs-section").classList.remove("d-none");

            if (containerS) {
                containerS.innerHTML = '<div class="text-white-50">Cargando subscripciones...</div>';
            }

            try {
                // Convertimos uids a array de strings
                const subsUIDs = Object.keys(subs.uids); 
                console.log(subsUIDs);
                // Filtrar animes
                const subsData = fullAnimeList.filter(anime =>
                    subsUIDs.includes(String(anime.unit_id))
                );

                if (!subsData.length) {
                    containerS.innerHTML = '<p class="text-white-50">Aún no has agregado animes a subscripciones.</p>';
                    hideLoader();
                    return;
                }
                ...
                console.error(err);
                containerS.innerHTML = '<p class="text-danger">Error mostrando subscripciones.</p>';
            }
...
async function searchSuggestionsInput() {
    ...
    try {
        ...
        if (lastSearchTerm !== value) return;
...
async function searchAnime(event = null, term = null) {
    ...
    try {
        ...
        apiResults = data.data
            .filter(a => {
                ...
                return terms.every(term => titles.some(t => t.includes(term)));
            })
            .map(a => ({
                title: a.attributes.titles?.en_jp,
    ...
    const seen = new Set();
    let combinedResults = [];
    for (const anime of localResults) {
        const norm = normalizeText(anime.title);
        if (!seen.has(norm)) {
            seen.add(norm);
    ...
    if (localResults.length === 0 && apiResults.length > 0) {
        for (const apiAnime of apiResults) {
            const titlesToCheck = [
                apiAnime.title,
                apiAnime.canonicalTitle
            ].filter(Boolean).map(normalizeText);

            const matches = fullAnimeList.filter(localAnime =>
                titlesToCheck.some(t => normalizeText(localAnime.title) === t)
            );

            for (const match of matches) {
                const norm = normalizeText(match.title);
                if (!seen.has(norm)) {
                    seen.add(norm);
                    combinedResults.push({ ...match, from: "local+kitsu" });
                }
            }
        }
    }

    search_selecion(combinedResults, inputNormalized);
}
...
async function animeInfo(uid) {
    ...
    // Variable para episodios y última emisión
    let episodes = [];
    ...
    // Descripción con animación de "Cargando..."
    ...
    (async () => {
        try {
            ...
            if (response.ok) {
                ...
                modalDescription.textContent = 'No se pudo cargar la descripción.';
            }
        } catch (err) {
    ...
    // Función para validar imagen negra
    ...
    // Cargar episodios y botón de notificaciones
    (async () => {
        ...
        if (episodesList) {
            ...
            <div style="display:flex;align-items:center;width:100%;gap:10px;">
                ...
                ">${texto}</button>
                <button class="notify-btn" id="notify-btn" style="
                    background:#ffffff;
                    border:none;
                    padding:6px 10px;
                    border-radius:6px;
                    ...
                    width:45px;
                    height:40px;
                    ${!isEmision ? 'display:none;' : ''}
                ">
                    <svg class="icon-unactive" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" style="width:1.6rem;height:1.6rem;fill:currentColor;">
            ...
            // IIFE para actualizar estado del botón según subs
            (async () => {
                try {
                    ...
                    const suscrito = subs?.uids?.[uid] !== undefined;
                    toggleNotifyIcon(notifyBtn, suscrito);
                } catch (err) {
                    console.error("[AUTO CHECK] Error al actualizar botón de notificación:", err);
                }
            })();
            ...
            if (notifyBtn) {
                notifyBtn.addEventListener('click', async (e) => {
                    ...
                    try {
                        if (subs.uids && subs.uids[uid] !== undefined) {
                            // Ya existe → anular
                            await anulSubcription(uuid, uid); // <-- Aseguramos que termine
                            toggleNotifyIcon(notifyBtn, false);
                        } else {
                            // No existe → añadir
                            await addSubcription(uuid, uid, lastEp); // <-- Aseguramos que termine
                            toggleNotifyIcon(notifyBtn, true);
                        }

                        // Actualizamos subs DESPUÉS de que se complete la acción
                        subs = await getSucriptions(uuid);
                        console.log("[NOTIFY] Subs actualizados:", subs);
                    } catch (err) {
                        console.error("[NOTIFY] Error al manejar la suscripción:", err);
                    }
                });
            }
            ...
            episodes.forEach(ep => {
                ...
                btn.addEventListener('click', () => window.location.href = `./player?uid=${encodeURIComponent(data.unit_id)}&ep=${ep.number}`);
...
/**
 * Cambia el icono del botón de notificación
 * @param {HTMLElement} notifyBtn - El botón donde cambiar el SVG
 * @param {boolean} [forceState] - Si se pasa, fuerza el estado true/false
 * @returns {boolean} El estado actual después del cambio
 */
...
function showContinueWatching() {
    ...
    if (dataAnime) {
        ...
        document.getElementById('continue-watching-btn').onclick = function () {window.location.href = `player?uid=${data.uid}&ep=${data.ep}`;};
...
function getPageParam() { const params = new URLSearchParams(window.location.search); const raw = params.get('page'); return /^[1-9]\d*$/.test(raw) ? parseInt(raw, 10) : 1; }
