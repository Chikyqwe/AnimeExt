...
function mainInit() {
    ...
    if (navInicio) navInicio.addEventListener('click', (e) => layout(1, e));
    const navDirectorio = document.getElementById('navDirectorio');
    if (navDirectorio) navDirectorio.addEventListener('click', (e) => layout(2, e));
    const navHistorial = document.getElementById('navHistorial');
    if (navHistorial) navHistorial.addEventListener('click', (e) => layout(3, e));
    const navFavoritos = document.getElementById('navFavoritos');
    if (navFavoritos) navFavoritos.addEventListener('click', (e) => layout(4, e));
    // Carga principal
    ...
    (async () => {
        try {
            const response = await fetch('/api/description', {
                ...
                body: JSON.stringify({ uid: data.unit_id })
            });
            if (response.ok) {
    ...
    (async () => {
                ...
                try {
                    ...
                    if (epResponse.ok) {
                        ...
                        if (result.episodes && Array.isArray(result.episodes.episodes) && result.episodes.episodes.length > 0) {
                            let rawepisodes = result.episodes.episodes;
                            if (rawepisodes.length < 50) { rawepisodes.reverse(); }
        ...
        if (episodesList) {
            ...
            <div style="display:flex;align-items:center;width:100%;gap:10px;">
                <button type="button" class="episode-status status episode-card" style="
                    flex:1;
                    background-color:${color} !important;
                    cursor:default;
                    font-weight:600;
                    color:#fff;
                    ${isEmision ? 'overflow:hidden;text-overflow:ellipsis;' : ''}
                ">${texto}</button>
            </div>`;
            ...
            episodes.forEach(ep => {
                ...
                btn.innerHTML = `
                    <div class="ep-thumb">
                        ...
                        <span class="ep-number">Episodio ${ep.number}</span>
                    </div>
                `;

                btn.addEventListener('click', () => {
                    window.location.href = `./player?uid=${encodeURIComponent(data.unit_id)}&ep=${ep.number}`;
                });
